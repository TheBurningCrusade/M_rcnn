    ...
    def create_roidb_from_box_list(self, box_list, gt_roidb):
        ...
        for i in range(self.num_images):
            ...
            if gt_roidb is not None and gt_roidb[i]['boxes'].size > 0:
                ...
                # 这里的gt_class是按一个图有多少个物体，每个物体的裂变编号存储的
                ...
                # 得到一行数据中元素最大的数的列索引,其实是数组的下标表示行号，数值是最大值的列号 
                argmaxes = gt_overlaps.argmax(axis=1) 
                maxes = gt_overlaps.max(axis=1) # 的到一行数据中元素最大的数的数值
                I = np.where(maxes > 0)[0] # 所有最大值大于0的行的行号
                # I:符合条件的行号, 重叠值最大的class（类别号） = 重叠度
                overlaps[I, gt_classes[argmaxes[I]]] = maxes[I]

            overlaps = scipy.sparse.csr_matrix(overlaps)
            # roidb整个的内容都是关于select_search_roi的, 注意这里的gt_classes元素都是0
