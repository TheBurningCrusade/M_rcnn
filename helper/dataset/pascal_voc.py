...
class PascalVOC(IMDB):
    ...
    def load_image_set_index(self):
        ...
        默认从/home/joker/research_hobby/no_one/mxnet/example/rcnn/data/VOCdevkit/VOC2007/ImageSets/Main/trainval.txt
        读取一系列id(str)
        :return:
        """
    ...
    def gt_roidb(self):
        ...
        #******************
        #print  "test gt_roidb"
        #x = self.load_pascal_annotation("000009")
        #print x
        #******************
        """
        gt_roidb(list): 一个元素表示一张图片的存储结构如下
    ...
    def load_pascal_annotation(self, index):
        ...
        #/home/joker/research_hobby/no_one/mxnet/example/rcnn/data/VOCdevkit/VOC2007/Annotations/000009.xml
        #print "xml file"
        #print filename
        ...
        if not self.config['use_diff']:
            ...
            objs = non_diff_objs
        num_objs = len(objs)    # num_objs表示的是一个图片上的object的数量
        # boxes 是标记一个object位置的数据
    ...
    def load_selective_search_roidb(self, gt_roidb):
        ...
        """
        # print "call load_sel"
        # matfile = /home/joker/research_hobby/no_one/mxnet/example/rcnn/data/selective_search_data/voc_2007_trainval.mat
        ...
        boxes: <type 'numpy.ndarray'>
        __version__: <type 'str'>
        __header__: <type 'str'>
        __globals__: <type 'list'>
        raw_data_dict = scipy.io.loadmat(matfile)
        ...
        #××××××××××××××××××××××××××××××××××
        #raw_data_dict = scipy.io.loadmat(matfile)
        #print raw_data_dict["images"].shape    # (5011, 1)
        #print "dccccccccccccc"
        #print raw_data_dict["images"][0][0]    #这里存的是每个图片的编号,和image_set_index是一样的
        #print raw_data_dict["boxes"].shape    #(1, 5011), boxes是一个有1行5011列的数组，其中每个元素是np.array类型的数组，这个数组也是一个二维数组， (2443, 4)
        #print type(raw_data_dict["boxes"][0][0])
        #print raw_data_dict["boxes"].ravel()[0]    # 进过ravel之后，boxes变成一个有5011元素的数组，其中每个数组中的元素仍然是一个(2443, 4)二位数组
        box_list = []
        # print "verify point*****"
        # print raw_data[3][1]
        for i in range(raw_data.shape[0]):    # 对5011 个二位array进行处理
            # 对每个array改变索引，并减去1, boxes任然是一个(2443,4)的数组
        ...
        # 对已每一个图（共5011）每个图都对应一个过滤完的list，list中存储一个二维数组
