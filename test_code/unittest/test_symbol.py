...
def test_symbol_internal():
    ...
    # print internal.tojson()
    # print internal.debug_str()
...
def test_symbol_infer_shape():
    ...
    # infer_shape返回的是一个含有3个元素的tuple，依次为所有的参数,所有的输出,所有的辅助
    ...
    """print "out_shapes: %s" % out_shapes # 输出矩阵shape  [(10L, 128L)]
    print "aux_shapes: %s" % aux_shapes
    print "arg:%s" % arg
    # 返回参数名['data', 'x2h_weight', 'x2h_bias', 'prevstate', 'h2h_weight', 'h2h_bias']
    print "list_arguments: %s" % out.list_arguments()"""
    ...
    arg_shapes = dict(zip(out.list_arguments(), arg))
    """print "arg: %s" % arg #[(10L, 64L), (128L, 64L), (128L,), (10L, 128L), (128L, 128L), (128L,)]
    print "list_arguments: %s" % out.list_arguments() #['data', 'x2h_weight', 'x2h_bias', 'prevstate', 'h2h_weight', 'h2h_bias']"""
    assert arg_shapes['data'] == (num_sample, num_dim)
    assert arg_shapes['x2h_weight'] == (num_hidden, num_dim)
...
def test_symbol_infer_shape_var():
    ...
    """print "arg_shaps:%s" % arg_shapes
    print "out_shapes:%s" % out_shapes
    print "out_shapes:%s" % aux_shapes"""
