...
def check_elementwise_sum_with_shape(shape, n):
    """mx.symbol.elementwisesum是对它的输入的元素进行累加，比如输入是[1, 2, 3] 和[4, 5, 6], 那么
    结果为[5, 7, 9]; 这是经过forward后才会执行计算;
    在本次测试backward时，是人工制定了残差的值，这样才能算他们各自的偏导. 因为结果是[5, 7, 9] 假设
    残差是[1.2, 8.5, 7.3]，那么[1,2,3] [4, 5, 6]他们求偏导之后的残差是[1.2, 8.5, 7.3] 和[1.2, 8.5, 7.3]
    c = a + b  那么a求偏导是1， b也是1
    """
    print "shape: %s" % (shape,)
    print "n: %s" % n
    ...
    exec1.forward()
    out1 = exec1.outputs[0].asnumpy()
    print "after forward out1: %s" % (out1,)
    out = sum(a.asnumpy() for a  in arr)
    print "out: %s" % (out,)
    assert reldiff(out, out1) < 1e-6
    out_grad = mx.nd.empty(shape)
    out_grad[:] = np.random.uniform(-10, 10, shape)
    print "out_grad: %s" % out_grad.asnumpy()
...
def test_elementwise_sum():
    ...
    check_elementwise_sum_with_shape((3,4), 2)
    """np.random.seed(0)
    nrepeat = 2
    maxdim = 4
    for repeat in range(nrepeat): # 2
        for dim in range(1, maxdim): # 3
            shape = tuple(np.random.randint(1, int(1000**(1.0/dim)), size=dim))
            # print "int(1000**(1.0/dim)): %f" % int(1000**(1.0/dim))
            # print shape
            check_elementwise_sum_with_shape(shape, np.random.randint(1, 8))"""

def check_slice_channel(dim, num):
    """按制定的格式对数据进行分割，如果求偏导的时候，残差不变
    """
    ins = []
    if dim == 2:
        shape = (2, 2)
    ...
    print "aux_shape: %s" % (aux_shape,)
    print "befor shape: %s" % (shape,)
    grad_nd = [mx.nd.empty(shape) for shape in arg_shape] # 注意这里的shape 和函数开头的shap重复了，这会对他从新赋值
    # print len(grad_nd)
    # print "grad_nd: %s" % (grad_nd[0].asnumpy(),)
    print "after shape: %s" % (shape, )
    ...
    print grad_nd[0].asnumpy()
    print np.hstack([ins[i] + i for i in range(num)])
    ...
    exe = op.bind(mx.cpu(), args=[e_nd], args_grad=grad_nd)
    exe.forward()
    print len(exe.outputs)
    print "output"
    for i in range(len(exe.outputs)):
        print exe.outputs[i].asnumpy()
...
def check_softmax_with_shape(shape, xpu):
    ...
    exec1.backward()
    # 这里就是经过反向传播之后的对于一个输出单元的输入求的导数，即对x的某一行一列元素的求导
    # 在正常情况下，这一列元素是前一层隐藏单元所有输出的累加和
