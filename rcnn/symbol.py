...
#注意这里pad=1是为了保证进行过卷积操作之后，图像的大小没有发生变化；同时在进行max pooling
#的时候，图像的缩放是有stride决定的，该函数有5个group，4个stride=2的max_pooling,所以图像
#被缩小了1/16=0.0625倍
...
def get_vgg_rcnn(num_classes=21):
    ...
    # Fast R-CNN
    # 图像被缩小了1/16倍，所以spatial_scale=0.0625;对一个feature map它在进行ROIPooling的时候，由于
    # 会有一系列的rois在一张图像上，所以pool5的中的batch_index变化
